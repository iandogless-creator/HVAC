...
project = _project()
before = repr(project)
resolved = build_committed_room_mean_radiant_from_project_state_v1(project)
repeated = build_committed_room_mean_radiant_from_project_state_v1(project)
assert resolved == repeated
assert repr(project) == before
assert resolved.ready
assert resolved.room_count == 2
assert resolved.blockers == ()
assert tuple(row.room_id for row in resolved.rooms) == ("room-a", "room-b")
...
assert math.isclose(
    rows["room-b"].mean_radiant_temperature_C,
    17.736,
    rel_tol=0.0,
    abs_tol=1.0e-12,
)
assert rows["room-a"].surface_count == 2
assert rows["room-b"].surface_count == 1
assert "area-fraction weighting" in rows["room-a"].source
assert "not a geometric view-factor solution" in resolved.note

# Ventilation evidence cannot feed back into surface temperature or Tri.
qv_changed = _project()
...
assert (
    build_committed_room_mean_radiant_from_project_state_v1(qv_changed)
    == resolved
)

stale = build_committed_room_mean_radiant_from_project_state_v1(
...
# N3A1 freshness blockers propagate through N3D2 and N3D4 unchanged.
temperature_stale_project = _project()
temperature_stale_project.rooms["room-a"].internal_temp_override_C = 21.0
temperature_stale = build_committed_room_mean_radiant_from_project_state_v1(
...
# N3D1 surface classification blockers also propagate unchanged.
unknown_class_project = _project()
unknown_class_project.heatloss_results["fabric"][0]["surface_class"] = "mystery"
unknown_class = build_committed_room_mean_radiant_from_project_state_v1(
...
# Persisted accepted-fabric container shape resolves identically.
persisted = _project()
persisted.heatloss_results["fabric"] = {"rows": _fabric_rows()}
assert (
    build_committed_room_mean_radiant_from_project_state_v1(persisted)
    == resolved
)

print(
    "OK — H-S66-N3D4 live ProjectState N3D2-to-N3D3 orchestration "
    "producing fresh room Tri evidence passed."
)
